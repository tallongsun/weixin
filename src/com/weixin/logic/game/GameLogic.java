package com.weixin.logic.game;

import java.util.ArrayList;
import java.util.HashMap;
import java.util.List;

import com.weixin.db.dao.GameDao;
...
public class GameLogic {
	...
	public static BaseResMessage process(String content,String openId){
		...
		// 获取用户最近一次创建的游戏
		Game game = GameDao.getLastGame(openId);
		// 本局游戏的正确答案
		...
		GameDao.saveGameRound(gameRound);
		// 查询本局游戏的所有回合
		List<GameRound> roundList = GameDao.findAllRoundByGameId(gameId);
		// 遍历游戏回合，组装给用户回复的消息
		...
		if ("4A0B".equals(guessResult)) {// 猜对
			...
			GameDao.updateGame(gameId, 1, TimeUtil.getStdDateTime());
		}else if (roundList.size() >= 10) {// 10回合仍没猜对
			buffer.append("正确答案：").append(gameAnswer).append("\n");
			buffer.append("唉，10回合都没猜出来，本局结束！[流泪]").append("\n");
			buffer.append("重新输入4个不重复的数字开始新的一局。");
			GameDao.updateGame(gameId, 2, TimeUtil.getStdDateTime());
		}else {// 本回合没猜对
			buffer.append("请再接再励！");
		}
	...
	public static String getUserScore(String openId) {
		StringBuffer buffer = new StringBuffer();
		HashMap<Integer, Integer> scoreMap = GameDao.getScoreByOpenId(openId);
