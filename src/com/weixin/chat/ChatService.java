...
public class ChatService {
	...
	public static String chat(String req,String openId){
		...
		if (null != knowledge) {
			if (2 == knowledge.getCategory()) {// 笑话
				answer = DBUtil.getJoke();
				chatCategory = 2;
			} else if (3 == knowledge.getCategory()) {// 上下文
				// 判断上一次的聊天类别
				int category = DBUtil.getLastCategory(openId);
				...
				if ("".equals(answer))
					answer = DBUtil.getKnowledSub(knowledge.getId());
				chatCategory = 1;
			}
		} else {// 未找到匹配项
		...
		// 保存聊天记录
		DBUtil.saveChatLog(openId, new Date().getTime()+"", req, answer, chatCategory);
	...
	private static void createIndex(File indexDir) {
		// 取得问答知识库中的所有记录
		List<Knowledge> knowledgeList = DBUtil.findAllKnowledge();
